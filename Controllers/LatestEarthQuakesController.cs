...
{
    public class LatestEarthQuakesController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            List<string> LatestToday = new List<string>();
            List<string> LatestMor = new List<string>();
            List<string> LatestAft = new List<string>();

            foreach(var c in Results)
            {
                if (c.Contains("Jan") && DateTime.Now.Month == 1 && DateTime.Now.Day == Convert.ToInt32(c.Substring(0, 1)) ||
                    c.Contains("Feb") && DateTime.Now.Month == 2 && DateTime.Now.Day == Convert.ToInt32(c.Substring(0, 1)) ||
                    c.Contains("Mar") && DateTime.Now.Month == 3 && DateTime.Now.Day == Convert.ToInt32(c.Substring(0, 1)) || 
                    c.Contains("Apr") && DateTime.Now.Month == 4 && DateTime.Now.Day == Convert.ToInt32(c.Substring(0, 1)))
                {
                    if (c.Contains("am"))
                    {
                        LatestMor.Add(c);
                    }
                    else if (c.Contains("pm"))
                    {
                        LatestAft.Add(c);
                    }
                }
            ...
            LatestToday.AddRange(LatestMor);
            LatestToday.AddRange(LatestAft);
            List<string> Info = GetInfoResults(document);
            ViewData["Potresi"] = LatestToday;
            ViewData["Info"] = Info;
        ...
        private List<string> AllEarthQuakesIznad3(IHtmlDocument document)
        {
            ...
            PotresiMagnituzaIznadTri(returnAllEarthQuakes, allEarthQuakesMag1, allEarthQuakesMag2, allEarthQuakesMag3, allEarthQuakesMag4, allEarthQuakesMag5, allEarthQuakesMag6);

            return returnAllEarthQuakes;
        }

        private static void PotresiMagnituzaIznadTri(List<string> returnAllEarthQuakes, IEnumerable<IElement> allEarthQuakesMag1, IEnumerable<IElement> allEarthQuakesMag2, IEnumerable<IElement> allEarthQuakesMag3, IEnumerable<IElement> allEarthQuakesMag4, IEnumerable<IElement> allEarthQuakesMag5, IEnumerable<IElement> allEarthQuakesMag6)
        {
            ...
            returnAllEarthQuakes.Add("Zadnji potresi iznad magnitude 2");
            returnAllEarthQuakes.Add("Vrijeme potresa, magnituda, dubina potresa, država, lokacija, grad");
            ...
            returnAllEarthQuakes.Add("Zadnji potresi iznad magnitude 1");
            returnAllEarthQuakes.Add("Vrijeme potresa, magnituda, dubina potresa, država, lokacija, grad");
